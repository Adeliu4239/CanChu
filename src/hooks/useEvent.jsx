...
import { apiEventGet, apiEventRead } from '@/api/api';
...
export default function useEvent() {
  ...
  const fetchEvent = async () => {
    try {
      ...
      const unreadEventCounts = response.data.data.events.reduce((count, event) => {
        return event.is_read === 0 ? count + 1 : count;
      }, 0);
      console.log('unreadEventCounts', unreadEventCounts);
      const unreads = response.data.data.events.filter((event) => !event.is_read);
  ...
  const handleEventRead = async (id) => {
    try {
      ...
      setUnreadEvents((prevUnreadEvents) => prevUnreadEvents.filter((event) => event.id !== id));
    } catch (err) {
  ...
  return { events, eventCounts, unreadEvents, loading, fetchEvent, handleEventRead };
}
